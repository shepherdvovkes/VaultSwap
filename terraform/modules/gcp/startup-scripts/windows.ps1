# GCP Windows Startup Script for VaultSwap DEX Infrastructure
...
# Enable logging
Start-Transcript -Path "C:\ProgramData\Google\Compute Engine\startup-script.log" -Append

Write-Host "Starting VaultSwap DEX Windows instance setup on GCP..."
...
choco install -y google-cloud-sdk
...
Invoke-WebRequest -Uri $gcloudUrl -OutFile $gcloudPath
Start-Process -FilePath $gcloudPath -ArgumentList "/S" -Wait
...
[Environment]::SetEnvironmentVariable("GCP_REGION", "${region}", "Machine")
[Environment]::SetEnvironmentVariable("NODE_ENV", "production", "Machine")
[Environment]::SetEnvironmentVariable("DATABASE_URL", "postgresql://${db_user}:${db_password}@${db_host}:5432/${db_name}", "Machine")
...
Invoke-WebRequest -Uri $opsAgentUrl -OutFile "$env:TEMP\add-google-cloud-ops-agent-repo.ps1"
...
$opsAgentConfig | Out-File -FilePath "C:\Program Files\Google\Cloud Operations\Ops Agent\config\config.yaml" -Encoding UTF8

# Start Google Cloud Ops Agent
Start-Service "Google Cloud Operations Agent"
...
REM Send to Google Cloud Logging
if exist "C:\Program Files (x86)\Google\Cloud SDK\google-cloud-sdk\bin\gcloud.cmd" (
    "C:\Program Files (x86)\Google\Cloud SDK\google-cloud-sdk\bin\gcloud.cmd" logging write vaultswap-metrics "CPU: %CPU_USAGE%%, Memory: %MEM_USAGE%%, Disk: %DISK_USAGE%%" --severity=INFO
)
"@
...
# Signal completion
New-Item -ItemType File -Path "C:\ProgramData\Google\Compute Engine\startup-script-complete.flag" -Force
